 ...
 * Burak Akguel, Christian Gesse, Fabian Ruhland, Filip Krakowski, Michael
 * Schoettner
 *
...
Terminal::Terminal(uint16_t columns, uint16_t rows)
    : outputStream(*this), columns(columns), rows(rows) {
...
void Terminal::write(uint8_t c) {
  ...
  if (isEscapeActive) {
    if (escapeEndCodes.contains(c)) {
      auto escapeSequence =
          currentEscapeSequence.substring(2, currentEscapeSequence.length());
      switch (c) {
      case 'A':
      case 'B':
      case 'C':
...
void Terminal::write(const uint8_t *sourceBuffer, uint32_t offset,
                     uint32_t length) {
...
int16_t Terminal::read() { return inputStream.read(); }

int32_t Terminal::read(uint8_t *targetBuffer, uint32_t offset,
                       uint32_t length) {
  return inputStream.read(targetBuffer, offset, length);
}

void Terminal::handleBell() {
  Async::Thread::createThread(
      "Terminal-Bell", new Async::FunctionPointerRunnable([]() {
...
void Terminal::handleTab() {
  if (getCurrentColumn() + TABULATOR_SPACES >= getColumns()) {
    ...
    setPosition(((getCurrentColumn() + TABULATOR_SPACES) / TABULATOR_SPACES) *
                    TABULATOR_SPACES,
                getCurrentRow());
  }
}
...
void Terminal::parseColorEscapeSequence(const Util::String &escapeSequence) {
  ...
  for (uint32_t i = 0; i < codes.length(); i++) {
    ...
    if (code < 30) {
      ...
      foregroundBaseColor =
          getColor(code - 30, Util::Graphic::Colors::WHITE, codes, i);
      brightForeground = false;
    } else if (code < 50) {
      backgroundBaseColor =
          getColor(code - 40, Util::Graphic::Colors::BLACK, codes, i);
      brightBackground = false;
    } else if (code < 98) {
      foregroundBaseColor =
          getColor(code - 90, Util::Graphic::Colors::WHITE, codes, i);
      brightForeground = true;
    } else if (code < 108) {
      backgroundBaseColor =
          getColor(code - 100, Util::Graphic::Colors::BLACK, codes, i);
...
void Terminal::parseCursorEscapeSequence(const Util::String &escapeSequence,
                                         char endCode) {
  switch (endCode) {
  case 'A': {
  ...
  case 'C': {
    const auto column =
        getCurrentColumn() + Util::String::parseInt(escapeSequence);
    setPosition(column < getColumns() ? column : getColumns() - 1,
                getCurrentRow());
    break;
  }
  case 'D': {
  ...
  case 'E': {
    const auto row =
        getCurrentRow() + Util::String::parseInt(escapeSequence) + 1;
  ...
  case 'n': {
    if (Util::String::parseInt(escapeSequence) == 6) {
      auto positionString = Util::String::format(
          "\u001b[%u;%uR", getCurrentRow(), getCurrentColumn());
      outputStream.write(reinterpret_cast<const uint8_t *>(
                             static_cast<const char *>(positionString)),
...
void Terminal::parseEraseSequence(const Util::String &escapeSequence,
                                  char endCode) {
  const auto code =
      escapeSequence.isEmpty() ? 0 : Util::String::parseInt(escapeSequence);
...
uint16_t Terminal::getColumns() const { return columns; }

uint16_t Terminal::getRows() const { return rows; }

Util::Graphic::Color
Terminal::getColor(uint8_t colorCode, const Util::Graphic::Color &defaultColor,
                   const Util::Array<Util::String> &codes, uint32_t &index) {
  switch (colorCode) {
  case 0:
...
Util::Graphic::Color
Terminal::parseComplexColor(const Util::Array<Util::String> &codes,
                            uint32_t &index) {
  ...
  default:
    Util::Exception::throwException(Util::Exception::INVALID_ARGUMENT,
                                    "Ansi: Invalid color mode!");
  }
}

Util::Graphic::Color
Terminal::parse256Color(const Util::Array<Util::String> &codes,
                        uint32_t &index) {
...
Util::Graphic::Color
Terminal::parseTrueColor(const Util::Array<Util::String> &codes,
                         uint32_t &index) {
...
void Terminal::setEcho(bool enabled) { echo = enabled; }

void Terminal::setLineAggregation(bool enabled) { lineAggregation = enabled; }

void Terminal::setAnsiParsing(bool enabled) { ansiParsing = enabled; }
...
void Terminal::TerminalPipedOutputStream::write(uint8_t c) {
  if (terminal.ansiParsing && c == '\b') {
    if (!lineBufferStream.isEmpty()) {
      ...
      if (terminal.echo) {
        terminal.putChar(' ', terminal.foregroundColor,
      ...
      auto line = lineBufferStream.getContent().substring(
          0, lineBufferStream.getLength() - 1);
      lineBufferStream.reset();
      lineBufferStream.write(static_cast<const uint8_t *>(line), 0,
                             line.length());
    }
  } else {
    if (terminal.echo) {
...
void Terminal::TerminalPipedOutputStream::write(const uint8_t *sourceBuffer,
                                                uint32_t offset,
                                                uint32_t length) {
  for (uint32_t i = 0; i < length; i++) {
    write(sourceBuffer[offset + i]);
...
void Terminal::TerminalPipedOutputStream::flush() {
  PipedOutputStream::write(
      static_cast<uint8_t *>(lineBufferStream.getContent()), 0,
      lineBufferStream.getLength());
  lineBufferStream.reset();
}

Terminal::KeyboardRunnable::KeyboardRunnable(Terminal &terminal)
    : terminal(terminal) {}
