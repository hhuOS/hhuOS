...
void Scene::updateEntities(double delta) {
    for (auto *entity : getEntities()) {
        reinterpret_cast<Util::Pulsar::D2::Entity*>(entity)->update(delta);
    }
}

void Scene::checkCollisions() {
    auto detectedCollisions = ArrayList<Pair<Util::Pulsar::D2::Entity*, Util::Pulsar::D2::Entity*>>();
    for (auto *entity : getEntities()) {
        auto *entity2D = reinterpret_cast<Util::Pulsar::D2::Entity*>(entity);
        if (entity2D->hasCollider() && entity2D->positionChanged) {
            const auto &collider = entity2D->getCollider();

            for (auto *otherEntity : getEntities()) {
                auto *otherEntity2D = reinterpret_cast<Util::Pulsar::D2::Entity*>(otherEntity);
