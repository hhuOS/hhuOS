...
#include "lib/util/pulsar/Entity.h"

namespace Util::Pulsar::D3 {

Entity::Entity(uint32_t tag, const Math::Vector3<double> &position, const Math::Vector3<double> &rotation, const Math::Vector3<double> &scale) : Util::Pulsar::Entity(tag), position(position), scale(scale), collider(Math::Vector3<double>(0, 0, 0), 0) {
    setRotation(rotation);
}

Entity::Entity(uint32_t tag, const Math::Vector3<double> &position, const Math::Vector3<double> &rotation, const Math::Vector3<double> &scale, const SphereCollider &collider) : Util::Pulsar::Entity(tag), position(position), scale(scale), colliderPresent(true), collider(collider) {
    setRotation(rotation);
}
