...
FileReader::FileReader(const Io::File &file) : fileDescriptor(Io::File::open(file.getCanonicalPath())) {
    if (fileDescriptor < 0) {
        Util::Exception::throwException(Exception::ILLEGAL_STATE, "FileReader: Unable to open file!");
    }
}

FileReader::FileReader(const String &path) : fileDescriptor(Io::File::open(path)) {
    if (fileDescriptor < 0) {
        Util::Exception::throwException(Exception::ILLEGAL_STATE, "FileReader: Unable to open file!");
    }
}
...
String FileReader::read(uint32_t length) {
    char *tmpBuffer = new char[length + 1];
    int32_t count = read(tmpBuffer, length);
    tmpBuffer[count] = 0;

    String ret = count <= 0 ? String() : String(tmpBuffer);
