...
FileWriter::FileWriter(const Io::File &file) : fileDescriptor(Io::File::open(file.getCanonicalPath())) {
    if (fileDescriptor < 0) {
        Util::Exception::throwException(Exception::ILLEGAL_STATE, "FileReader: Unable to open file!");
    }
}

FileWriter::FileWriter(const String &path) : fileDescriptor(Io::File::open(path)) {
    if (fileDescriptor < 0) {
        Util::Exception::throwException(Exception::ILLEGAL_STATE, "FileReader: Unable to open file!");
    }
}
...
void FileWriter::write(const String &string) {
    write(string, 0, string.length());
}

void FileWriter::write(const String &string, uint32_t offset, uint32_t length) {
