...
Tables::Tables(const Rsdt &rsdt) : tables(rsdt.getTableCount()) {
    for (size_t i = 0; i < getTableCount(); i++) {
...
Iterator<SdtHeader> Tables::begin() const {
    const auto element = IteratorElement<SdtHeader>{ tables[0], 0 };
    return Iterator<SdtHeader>(*this, element);
}

Iterator<SdtHeader> Tables::end() const {
    const auto element = IteratorElement<SdtHeader>{ tables[getTableCount()], getTableCount() };
    return Iterator<SdtHeader>(*this, element);
}
