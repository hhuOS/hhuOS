...
public:
    ...
    explicit Graphics(const Graphic::LinearFrameBuffer &lfb, double scaleFactor);
    ...
    void drawImage2D(const Math::Vector2<double> &position, const Graphic::Image &image, bool flipX = false,
        double alpha = 1, const Math::Vector2<double> &scale = Util::Math::Vector2<double>(1, 1),
    ...
    void drawCuboid3D(const Math::Vector3<double> &position, const Math::Vector3<double> &size,
        const Math::Vector3<double> &rotation, const D3::Texture &texture = D3::Texture()) const;

    /// Draw a colored 2D rectangle in 3D space. Works only in 3D scenes.
    void drawRectangle3D(const Math::Vector3<double> &position, const Math::Vector2<double> &size,
        const Math::Vector3<double> &rotation, const D3::Texture &texture = D3::Texture()) const;

    /// Draw a custom shape defined by vertices in 3D space. Works only in 3D scenes.
    void drawCustomShape3D(const Math::Vector3<double> &position, const Math::Vector3<double> &scale,
    ...
    void drawList3D(const Math::Vector3<double> &position, const Math::Vector3<double> &scale,
        const Math::Vector3<double> &rotation, GLuint list) const;
    ...
    /// List a colored cuboid for inclusion in a display list.
    static void listCuboid3D(const Math::Vector3<double> &translation, const Math::Vector3<double> &size,
    ...
    void clear(const Graphic::Color &color = Graphic::Colors::BLACK) const;
...
private:
    ...
    void drawImageDirect2D(const Math::Vector2<double> &position, const Graphic::Image &image,
        bool flipX, double alpha) const;

    void drawImageScaled2D(const Math::Vector2<double> &position, const Graphic::Image &image,
        bool flipX, double alpha, const Util::Math::Vector2<double> &scale) const;

    void drawImageRotated2D(const Math::Vector2<double> &position, const Graphic::Image &image,
        bool flipX, double alpha, double rotationAngle) const;

    void drawImageScaledAndRotated2D(const Math::Vector2<double> &position, const Graphic::Image &image,
        bool flipX, double alpha, const Math::Vector2<double> &scale, double rotationAngle) const;
