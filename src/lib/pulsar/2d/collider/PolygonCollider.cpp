...
Collision PolygonCollider::isColliding(PolygonCollider &other) {
    Collision smallestOverlap = { INT16_MAX, Util::Math::Vector2<double>() };
    ...
    for (uint32_t i = 0; i < polygon.getVertices().length(); i++) {
        ...
        if ((range.getFirst() - rangeOther.getSecond() > 0) || (rangeOther.getFirst() - range.getSecond() > 0)) {
            return { 0, Util::Math::Vector2<double>() };
        }
...
Util::Pair<double, double> PolygonCollider::projectPolygonOnAxis(Util::Array<Util::Math::Vector2<double>> vertices, const Util::Math::Vector2<double> &axis) {
...
Util::Math::Vector2<double> PolygonCollider::getAxes(Util::Array<Util::Math::Vector2<double>> vertices, uint32_t index) {
    auto point1 = vertices[index];
    auto point2 = index >= vertices.length() - 1 ? vertices[0] : vertices[index + 1];
    auto axis = Util::Math::Vector2<double>(-(point2.getY() - point1.getY()), point2.getX() - point1.getX());
