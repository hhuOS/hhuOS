...
LinearFrameBuffer::LinearFrameBuffer(void *address, uint16_t resolutionX, uint16_t resolutionY, uint8_t bitsPerPixel, uint16_t pitch) :
        buffer(static_cast<uint8_t*>(address)), resolutionX(resolutionX), resolutionY(resolutionY), bitsPerPixel(bitsPerPixel), pitch(pitch) {}
...
Color LinearFrameBuffer::readPixel(uint16_t x, uint16_t y) {
    ...
    uint8_t *ptr = buffer + (x * (bpp / 8)) + y * pitch;
