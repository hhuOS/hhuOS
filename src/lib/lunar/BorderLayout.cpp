...
void BorderLayout::arrangeWidgets(const Util::ArrayList<WidgetEntry> &widgets) const {
    ...
    for (auto &entry : widgets) {
        if (entry.args.length() == 0) {
            Util::Panic::fire(Util::Panic::INVALID_ARGUMENT, "BorderLayout: No position argument given!");
        }
...
size_t BorderLayout::getPreferredWidth(const Util::ArrayList<WidgetEntry> &widgets) const {
    size_t northWidth = 0;
    size_t southWidth = 0;
...
size_t BorderLayout::getPreferredHeight(const Util::ArrayList<WidgetEntry> &widgets) const {
