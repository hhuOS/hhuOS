...
void main() {
    ...
    auto nextId = idGenerator.getNextId();
            ...
            if (window.inputStream->isReadyToRead()) {
                ...
                switch (command) {
                    case Kepler::CREATE_WINDOW: {
                        ...
                        auto bufferSize = 320 * 240 * ((lfb.getColorDepth() + 7) / 8);
                    ...
                    case Kepler::FLUSH: {
                        ...
                        auto targetAddress = lfb.getBuffer().add((window.posY + font.getCharHeight() + 4) * lfb.getPitch() + (window.posX + 1) * ((lfb.getColorDepth() + 7) / 8));

                        for (uint16_t y = 0; y < window.resY; y++) {
                            targetAddress.copyRange(sourceAddress, window.resX * ((lfb.getColorDepth() + 7) / 8));
                            targetAddress = targetAddress.add(lfb.getPitch());
                            sourceAddress = sourceAddress.add(window.resX * ((lfb.getColorDepth() + 7) / 8));
                        }
