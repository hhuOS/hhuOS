...
void main() {
    ...
    while (true) {
            ...
            if (window.inputStream->isReadyToRead()) {
                ...
                switch (command) {
                    case FLUSH: {
                        ...
                        auto targetAddress = lfb.getBuffer().add((window.posY + 1) * lfb.getPitch() + (window.posX + 1) * ((lfb.getColorDepth() + 7) / 8));
        ...
        if (nextPipe->isReadyToRead()) {
            ...
            auto bufferSize = request.resX * request.resY * ((lfb.getColorDepth() + 7) / 8);
