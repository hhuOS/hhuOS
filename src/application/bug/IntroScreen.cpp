...
void IntroScreen::initializeBackground(Pulsar::Graphics &graphics) {
    ...
    for (uint32_t i = 0; i < lines; i++) {
        auto x = static_cast<uint16_t>(centerX - (Util::Address(INTRO_TEXT[i]).stringLength() * Pulsar::Graphics::FONT_SIZE) / 2.0);
        graphics.drawStringDirectAbsolute(x, y + i * Pulsar::Graphics::FONT_SIZE, INTRO_TEXT[i]);
    }
}
