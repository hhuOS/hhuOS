...
int32_t main(int32_t argc, char *argv[]) {
    ...
    if (args.length() < 1) {
        Util::System::error << "view3d: No arguments provided!" << Util::Io::PrintStream::lnFlush;
        return -1;
    }

    auto file = Util::Io::File(args[0]);
    if (!file.exists() || file.isDirectory()) {
        Util::System::error << "view3d: '" << args[0] << "' could not be opened!" << Util::Io::PrintStream::lnFlush;
        return -1;
    }
