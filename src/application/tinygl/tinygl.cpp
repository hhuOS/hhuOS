...
int32_t main(int32_t argc, char *argv[]) {
    ...
    if (arguments.length() == 0) {
        Util::System::error << "tinygl: No arguments provided! Please specify a demo (info, triangle, gears, cubes)." << Util::Io::PrintStream::lnFlush;
        return -1;
    }
    ...
    if (lfb.getColorDepth() != TGL_FEATURE_RENDER_BITS) {
        Util::System::error << "tinygl: Color depth not supported (Required: " << TGL_FEATURE_RENDER_BITS << ", Got: " << lfb.getColorDepth() << ")!" << Util::Io::PrintStream::lnFlush;
        return -1;
    }
