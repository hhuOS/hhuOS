...
void GameOverScreen::initializeBackground(Pulsar::Graphics &graphics) {
    ...
    auto y = static_cast<uint16_t>(centerY - ((lines * Pulsar::Graphics::FONT_SIZE) / 2.0));

    graphics.clear();
    graphics.setColor(Util::Graphic::Colors::GREEN);
    for (uint32_t i = 0; i < lines; i++) {
        auto x = static_cast<uint16_t>(centerX - (Util::Address(TEXT[i]).stringLength() * Pulsar::Graphics::FONT_SIZE) / 2.0);
        graphics.drawStringDirectAbsolute(x, y + i * Pulsar::Graphics::FONT_SIZE, i == 4 ? Util::String::format("You died at level %u!", level) : TEXT[i]);
    }
}
