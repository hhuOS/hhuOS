...
int32_t main(int32_t argc, char *argv[]) {
    ...
    for (const auto &path : arguments) {
        ...
        if (!file.exists()) {
            Util::System::out << "rmdir: '" << path << "' not found!" << Util::Io::PrintWriter::endl;
            continue;
        }

        if (!file.isDirectory()) {
            Util::System::out << "rmdir: '" << path << "' is not a directory!" << Util::Io::PrintWriter::endl;
            continue;
        }

        if (file.getChildren().length() > 0) {
            Util::System::out << "rmdir: '" << path << "' is not empty!" << Util::Io::PrintWriter::endl;
            continue;
        }

        auto success = file.remove();
        if (!success) {
            Util::System::out << "rmdir: Failed to delete directory '" << path << "'!" << Util::Io::PrintWriter::endl << Util::Io::PrintWriter::flush;
        }
    }
