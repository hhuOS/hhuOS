...
void Kernel::Usb::Driver::KernelMassStorageDriver::create_usb_dev_node() {
    ...
    for(int i = 0; i < MAX_DEVICES_PER_USB_DRIVER; i++){
        if(msd_driver->msd_map[i] == 0) continue;

        Util::String node_name = Util::String::format("msd%u", current_msd_node_num++);
        Kernel::Usb::UsbNode* msd_node = new Kernel::Usb::MassStorageNode(this, i, node_name);
        msd_node->add_file_node("");
        kernel_msd_logger.info("Succesful added msd node : minor %u -> associated with 0x%x (%s driver)...", msd_node->get_minor(), msd_driver, this->getName());
    }
}

bool Kernel::Usb::Driver::KernelMassStorageDriver::control(uint32_t request, const Util::Array<uint32_t>& parameters, uint8_t minor){
    MassStorageDev* msd_dev = driver->get_msd_dev_by_minor(driver, minor);
    if(msd_dev == (void*)0) return false;
    switch(request){
        case GET_SIZE : {
        ...
        case GET_INQUIRY : {
            ...
            if(driver->get_inquiry_data(driver, msd_dev, volume, param, (uint8_t*)user_address, (uint8_t*)user_address_len) == -1)
                return false;
            return true;    
        };
        case GET_CAPACITY : {
            ...
            if(driver->get_capacity_descpritor(driver, msd_dev, volume, capacity, param, (uint8_t*)user_address, (uint8_t*)user_address_len) == -1)
                return false;
            return true;  
        };
        case GET_SENSE : {
        ...
        case GET_READ_CAPACITY : {
            ...
            if(driver->get_capacity(driver, msd_dev, volume, param, (uint8_t*)user_address, (uint8_t*)user_address_len) == -1)
                return false;
            return true;
        };
...
uint64_t Kernel::Usb::Driver::KernelMassStorageDriver::readData(uint8_t *targetBuffer, uint64_t start_lba, uint64_t msd_data, uint8_t minor){
    ...
    return driver->read_msd(driver, targetBuffer, start_lba, blocks, magic, u_tag, volume, minor);
}

uint64_t Kernel::Usb::Driver::KernelMassStorageDriver::writeData(const uint8_t *sourceBuffer, uint64_t start_lba, uint64_t msd_data, uint8_t minor){
    uint32_t blocks = (msd_data & 0xFFFFFFFF00000000) >> 32;
    uint8_t volume  = msd_data & 0xFF;
    uint16_t magic  = (msd_data & 0xFFFF00) >> 8;
    uint8_t u_tag   = (msd_data & 0xFF000000) >> 24;

    return driver->write_msd(driver, (uint8_t*)sourceBuffer, start_lba, blocks, magic, u_tag, volume, minor);
}