...
void CpuService::prepareGdts() {
    ...
    // Restore bootstrap GDT and TSS
    auto bootstrapCpuId = registerCpu(getLocalApicId());
    gdt[bootstrapCpuId] = bootstrapGdt[0];
    tss[bootstrapCpuId] = bootstrapTss[0];
    delete bootstrapGdt;
...
void CpuService::prepareApplicationProcessorStartupCode() {
    ...
    boot_ap_gdts = reinterpret_cast<uint32_t>(gdt);
