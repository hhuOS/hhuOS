...
void new_super_usb_controller(UsbController *controller,
                              SystemService_C *mem_service, const char *name) {
  ...
  controller->dispatcher =
      controller->request_event_dispatcher(controller, mem_service);
  controller->interface_dev_map =
      controller->request_interface_device_map(controller, mem_service);
  controller->controller_mutex =
      controller->request_mutex(controller, mem_service);
}

EventDispatcher *request_event_dispatcher(UsbController *controller,
                                          SystemService_C *mem_service) {
  EventDispatcher *dispatcher;
  MemoryService_C *m =
      (MemoryService_C *)container_of(mem_service, MemoryService_C, super);

  dispatcher = (EventDispatcher *)m->allocateKernelMemory_c(
      m, sizeof(EventDispatcher), 0);
...
Mutex_C *request_mutex(UsbController *controller,
                       SystemService_C *mem_service) {
  MemoryService_C *m =
      (MemoryService_C *)container_of(mem_service, MemoryService_C, super);
...
SuperMap *request_interface_device_map(UsbController *controller,
                                       SystemService_C *mem_service) {
  MemoryService_C *m =
      (MemoryService_C *)container_of(mem_service, MemoryService_C, super);
...
int supported_event_listener_type(UsbController *controller,
