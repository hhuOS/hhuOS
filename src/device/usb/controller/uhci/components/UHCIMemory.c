...
size_t io_write(struct Addr_Region *addr_region, void *b, uint8_t length,
  ...
  if (length == 0x01) {
    ...
    i_o_reg->io_port->writeDoubleWord_IO_off_C(i_o_reg->io_port, type,
                                               int32_tvalue);
    return length;
  }
  return 0;
...
size_t io_read(struct Addr_Region *addr_region, uint8_t length, void *r,
  ...
  if (length == 0x01) {
    ...
    *(int8_pvalue) =
        i_o_reg->io_port->readByte_IO_off_C(i_o_reg->io_port, type);
    return length;
  } else if (length == 0x02) {
    int16_pvalue = (uint16_t *)buffer;
    *(int16_pvalue) =
        i_o_reg->io_port->readWord_IO_off_C(i_o_reg->io_port, type);
    return length;
  } else if (length == 0x04) {
    int32_pvalue = (uint32_t *)buffer;
    *(int32_pvalue) =
        i_o_reg->io_port->readDoubleWord_IO_off_C(i_o_reg->io_port, type);
    return length;
  }
  return 0;
}
