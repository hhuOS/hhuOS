...
void new_super_event_listener(EventListener* listener){
    ...
    listener->listener_mutex = (Mutex_C*)interface_allocateMemory(sizeof(Mutex_C), 0);
    listener->listener_mutex->new_mutex = &new_mutex;
    listener->listener_mutex->new_mutex(listener->listener_mutex);
}

void register_event_callback(EventListener* event_listener, event_callback callback){
    list_element* l_e = event_listener->head.l_e;
    Event_Callback* event_c = (Event_Callback*)interface_allocateMemory(sizeof(Event_Callback), 0);
...
void deregister_event_callback(EventListener* event_listener, event_callback callback){
    list_element* l_e = event_listener->head.l_e;
    list_element* prev = 0;
    ...
    while(l_e != (void*)0){
        Event_Callback* event_c = (Event_Callback*)container_of(l_e, Event_Callback, l_e);
