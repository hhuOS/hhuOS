...
void BufferScroller::scrollUp(uint16_t lineCount) {
    ...
    destination.copyRange(source, lfb.getPitch() * lfb.getResolutionY() - lfb.getPitch() * lineCount);
...
void BufferScroller::scrollDown(uint16_t lineCount) {
    ...
    for (int32_t i = 1; i < lfb.getResolutionY() - lineCount; i++) {
        auto source = Util::Memory::Address<uint32_t>(lfb.getBuffer() + lfb.getPitch() * lfb.getResolutionY() - i * lfb.getPitch() - lineCount * lfb.getPitch());
        auto destination = Util::Memory::Address<uint32_t>(lfb.getBuffer() + lfb.getPitch() * lfb.getResolutionY() - (i - 1) * lfb.getPitch());
